...
Snakemake pipeline
- run rtg-tools vcfstats on benchmark VCFs, excluding variants outside benchmark regions
- get counts for autosomes and sex chromosomes separately, potentially by chromosome (additional compute upfront but eaiser analysis later)
...
## Using variant size from vcf files
...
%TODO% rtg stats for v4, v0.6, CMRG, TR
%TODO% counts excluding XY for more direct comparison to previous versions
...
bench_var_metrics_df <- bench_var_rtg_stat_files %>%
  ...
  select(-asm_varcall) %>%
  add_column(bench_version = "v5.0q", .before = "ref")
...
#   mutate(
#     var_length = as.integer(str_remove(var_length, "-.*")),
#     var_length = if_else(var_type == "Delete", -var_length, var_length)
#   )
# glimpse(var_len_fig_df)
```

Tibble data structure
```{r}
# %TODO% add data frame assertions in place of glimpse
glimpse(bench_var_metrics_df)
```
...
(
  stvar_cnt_table <- var_cnt_df %>%
    filter(bench_type == "stvar", metric %in% c("Insertions", "Deletions", "Indels")) %>%
    select(-bench_type) %>%
    pivot_wider(names_from = metric, values_from = value, values_fn = sum) %>%
...
%%TODO%% Combine and get overall totals

### Variant Size Distributions

```{r}
## TSV generate using make_sv_len.sh script
sv_len_df <- read_tsv("../sv_len.tsv", show_col_types = FALSE) %>%
  filter(SVTYPE != ".") %>%
  rename(CHROM = `#CHROM`) %>%
  mutate(
    SVLEN = as.integer(SVLEN),
    TRFstart = if_else(TRFstart == ".", 0, as.integer(TRFstart)),
    TRFend = if_else(TRFend == ".", -1, as.integer(TRFend)),
    TRF = if_else(TRF == 1, 1, 0),
    TRFlen = TRFend - TRFstart + 1
  )

sv_len_fig_df <- sv_len_df %>%
  filter(SVLEN > 49) %>%
  mutate(SVLEN = if_else(SVTYPE == "DEL", -SVLEN, SVLEN)) %>%
  mutate(TRcls = case_when(
    str_detect(RM_clsfam, "SINE/Alu") ~ "Alu",
    str_detect(RM_clsfam, "LINE/L1") ~ "LINE",
    RM_clsfam == "." ~ "Non-Repeat",
    TRUE ~ "Other Repeat"
  ))
...
ggplot(
  ...
  geom_histogram(
    data = sv_len_fig_df %>% filter(TRcls %in% c("Alu", "LINE")),
    aes(fill = TRcls),
    color = "grey20",
...
%%TODO%% generate stats files for benchmark set comparisons - using make_var_len.sh script, may need to annotate other VCFs.
...
regions_df <- c(bench_region_list, v5q_region_list) %>%
  ...
  mutate(
    ...
    var_type = case_when(
      ...
      str_detect(benchmarkset, "stvar") | str_detect(benchmarkset, "SV") ~ "stvar",
...
```
```{r}
# %TODO% add assertions to replace glimpse and FIXME checks
glimpse(regions_df)
regions_df %>% filter(bench_version == "FIXME" | var_type == "FIXME" | ref == "FIXME")
```
...
```{r}
hg002q100v1.1_size <- read_tsv(here("data/hg002v1.1.mat_Y_EBV_MT.fasta.gz.fai"),
...
regions_df %>%
  ...
  left_join(ref_size) %>% 
  mutate(chrom_group = if_else(chrom %in% paste0("chr", c("X", "Y")), "sexchrom", "autosome")) %>%
  group_by(var_type, ref, genome_size, chrom_group, bench_version) %>%
  summarise(total_bases = sum(interval_size), .groups = "drop") %>%
  pivot_wider(names_from = chrom_group, values_from = total_bases, values_fill = 0) %>% 
    mutate(total_bp = autosome + sexchrom,
