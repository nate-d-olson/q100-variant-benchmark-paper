...
(coverage_combined_plt <- (pct_context_plt +
  ...
  theme(legend.position = "bottom", margins = unit(c(0, 0, 0, 0), "pt")))
```
...
```{r variant_count_table, eval=FALSE}
genomic_context_metrics_df %>%
  filter(bench_version == "v5.0q") %>%
  mutate(
    ref = std_references(ref),
  ...
  arrange(context_label, ref, bench_type) %>%
  mutate(across(c(total_variants, snv_count, indel_count,
    del_count, ins_count, complex_count), ~ replace_na(., 0L))) %>%
  as_grouped_flextable(groupname_col = "context_label") %>%
...
```{r variant_counts_by_type_plot, fig.width = 8, fig.height = 6, eval=FALSE}
...
```{r variant_fold_change, fig.width = 8, fig.height = 5, eval=FALSE}
...
```{r variant_density_plot, fig.width = 8, fig.height = 5, eval=FALSE}
...
```{r relative_density_plot, fig.width = 8, fig.height = 5, eval=FALSE}
...
```{r coverage_vs_density_scatter, fig.width = 8, fig.height = 5, eval=FALSE}
