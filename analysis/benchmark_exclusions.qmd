...
interaction_filtered_tbl <- exclusion_interaction_tbl %>%
  ...
  dplyr::mutate(
    ...
    include_bases = pct_bases >= 1 | n_exclusions >= 2,
    include_variants = pct_variants >= 1 | n_exclusions >= 2
  ) %>%
  dplyr::ungroup()
```
