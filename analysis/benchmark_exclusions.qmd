...
load_exclusion_metrics() %>%
  ...
  # Merge bp + pct columns into "123,456 (0.005)" strings per reference
  mutate(
    GRCh37 = sprintf("%s (%.3f)", scales::comma(intersect_bp_GRCh37), pct_of_dip_GRCh37),
    GRCh38 = sprintf("%s (%.3f)", scales::comma(intersect_bp_GRCh38), pct_of_dip_GRCh38),
    CHM13v2.0 = sprintf(
      "%s (%.3f)",
      scales::comma(intersect_bp_CHM13v2.0), pct_of_dip_CHM13v2.0
    )
  ) %>%
  select(exclusions, bench_type, GRCh37, GRCh38, CHM13v2.0) %>%
  flextable() %>%
  flextable::align(j = c("exclusions", "bench_type"), align = "right", part = "body") %>%
...
if (nrow(single_exclusion_summary) > 0) {
  # Get reference names from column names
  refs <- unique(gsub("bases_bp_|variant_count_", "",
    grep("bases_bp_|variant_count_", names(single_exclusion_summary), value = TRUE)))

  # Reorder columns: group by reference (bases, variants per ref)
  ref_cols <- unlist(lapply(refs, function(r) {
    c(paste0("bases_bp_", r), paste0("variant_count_", r))
  }))

  single_exclusion_summary %>%
    select(exclusion_combination, bench_type, all_of(ref_cols)) %>%
    flextable() %>%
    fmt_integer_flextable(columns = ref_cols) %>%
    cols_label_flextable(exclusion_combination = "Exclusion", bench_type = "Type") %>%
    flextable::set_header_labels(
      values = setNames(rep(c("Bases", "Variants"), length(refs)), ref_cols)
    ) %>%
    flextable::add_header_row(
      values = c("", "", unlist(lapply(refs, function(r) c(r, "")))),
      top = TRUE
    ) %>%
    flextable::merge_h(part = "header") %>%
    flextable::set_caption(caption = "Single Exclusions Not Shown in Upset Plots") %>%
    theme_flextable_manuscript()
} else {
...
create_base_upset <- function(data, ref_name, bench_type_name, title_suffix = "") {
  ...
  ggplot(plot_data, aes(x = exclusions_list, y = bases_bp)) +
    ggupset::scale_x_upset(n_intersections = 30,reverse = TRUE) +
...
```{r}
 create_variant_upset(interaction_filtered_tbl, "GRCh38", "smvar")
```
...
interaction_summary %>%
  flextable() %>%
  flextable::set_caption(caption = "Exclusion Interaction Summary") %>%
  cols_label_flextable(
    ref = "Reference",
    bench_type = "Type",
