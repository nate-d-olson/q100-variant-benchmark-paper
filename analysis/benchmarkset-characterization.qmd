...
regions_df <- c(
  ...
  separate(benchmarkset, into = c("bench_version","ref","var_type"), sep = "_")
...
diff_coverage <- list.files(
  ...
  {
    set_names(., str_extract(., "(?<=coverage/).*(?=_cov.bed)") %>%
      str_replace("/", "_"))
  } %>%
  map_dfr(read_tsv,
    col_names = c("CHROM", "START", "END", "n_overlap", "bases_cov", "ivl_len", "frac_cov"),
    col_types = "ciiiiid",
    .id = "strat"
  ) %>% 
  filter(n_overlap > 0) %>% 
  separate(strat, into = c("bench_version","ref","var_type","strat"), sep = "_")
```
...
diff_cov_total %>% 
  ggplot() +
  geom_bar(aes(x = strat, y = total_cov, fill = bench_version), stat = "identity", position = "dodge") +
