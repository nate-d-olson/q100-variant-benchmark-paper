...
(smvar_len_dist_plt <-  ggplot(smvar_len_df,
  ...
  geom_histogram(
    ...
    fill = "grey80", color = "grey20",binwidth = 1, drop = "all"
  ) +
  geom_histogram(
    data = filter(smvar_len_df, bench_version == "v4.2.1"),
    fill = "grey30", color = "grey20", binwidth = 1, drop = "all"
  ) +
  scale_y_continuous(labels = scales::comma) +
  labs(
    x = "SV Length (bp)",
...
(diff_smvar_plt <- diff_var_plt_df %>%
  mutate(bench_version = factor(bench_version,
...
regions_df <- c(bench_region_list, v5q_region_list, pg_region_list) %>%
  ...
  mutate(
    ...
    bench_version = case_when(
      str_detect(benchmarkset, "v5q") ~ "v5.0q",
      str_detect(benchmarkset, "v421") ~ "v4.2.1",
      str_detect(benchmarkset, "v06") ~ "v0.6",
    ...
    ref = case_when(
      str_detect(benchmarkset, "grch37") ~ "GRCh37",
      str_detect(benchmarkset, "grch38") ~ "GRCh38",
      str_detect(benchmarkset, "chm13") ~ "CHM13v2.0",
      str_detect(benchmarkset, "v06") | str_detect(benchmarkset, "grch37") ~ "GRCh37",
