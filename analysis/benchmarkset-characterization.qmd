...
pg_region_list <- list(
  "platinum_pedigree_smvar_GRCh38" = here(
    "data/platinum-pedigree-data/NA12878_hq_v1.2.smallvar.bed.gz"
  ),
  "platinum_pedigree_stvar_GRCh38" = here(
...
regions_df <- c(
  ...
  mutate(
    ...
    var_type = case_when(
              str_detect(benchmarkset, "smvar") ~ "smvar",
        str_detect(benchmarkset, "stvar") ~ "stvar",
    str_detect(benchmarkset, "smallvar") ~ "smvar",
        str_detect(benchmarkset, "\\.svs\\.") ~ "stvar",
      TRUE ~ "FIXME"
    ),
    ref = case_when(
      ...
      str_detect(benchmarkset, "platinum_pedigree") ~ "GRCh38",
      TRUE ~ "FIXME"
    ),
...
```{r}
ref_size_df <- list.files(here("results/ref_genome_sizes"), pattern = "_size.tsv",full.names = TRUE) %>%
...
```{r}
total_ref_asm_bp <- ref_size_df |> group_by(ref) |> summarise(total_asm_bp = sum(asm_bp))
```
...
```{r}
strat_combined <- read_tsv(
  here("results/combine_stratification_beds/v5q_chm13_smvar/strat_combined.bed.gz"),
  col_names = c("chrom", "start", "end", "context"),
  col_types = "ciic"
  )
...
```{r}
strat_combined <- read_tsv(here("v5_grch38_strat.tsv"))
```

```{r}
strat_dip <- strat_combined %>% 
  filter(dip != 0)
```

```{r}
strat_bench <- strat_combined %>% 
  filter(bench == 1)
```
