...
(names(anno_var_tbl_files) <- anno_var_tbl_files %>%
  str_extract("(?<=var_tables/).*(?=/variants.tsv)") %>%
...
tidy_smvar <- function(var_tbl) {
  ...
  var_tbl <- var_tbl %>%
    mutate(var_type = case_when(
      ...
      var_type == "OTHER" & (len_ref > 1 & len_alt == 1) | (len_ref == 1 & len_alt > 1) ~ "INDEL",
      ## Assigning variant types for overlapping (atmoic) variants
      var_type == "OVERLAP" & len_ref == 1 & len_alt == 1 ~ "SNP",
      var_type == "OVERLAP" & (len_ref > 1 & len_alt == 1) | (len_ref == 1 & len_alt > 1) ~ "INDEL",
...
tidy_stvar <- function(var_tbl) {
  ...
  ## Annotating Variant Length
  var_tbl$var_size <- 0
  var_tbl$var_size[var_tbl$SVTYPE == "INS"] <- var_tbl$SVLEN[var_tbl$SVTYPE == "INS"]
  var_tbl$var_size[var_tbl$SVTYPE == "DEL"] <- -var_tbl$SVLEN[var_tbl$SVTYPE == "DEL"]
...
get_bench_var_cols <- function(benchmarkset) {
  ...
  cnames <- c(
    ...
    "len_ref", "len_alt", "strat_ids", "region_ids"
  )
  ctypes <- "ciiccciicc"

  if (str_detect(benchmarkset, "v0.6")) {
    cnames <- c(
      cnames, "END", "SVTYPE", "SVLEN", "ClusterIDs", "NumClusterSVs",
      "ExactMatchIDs", "NumExactMatchSVs", "ClusterMaxShiftDist",
      "ClusterMaxSizeDiff", "ClusterMaxEditDist",
      "PBcalls", "Illcalls", "TenXcalls", "CGcalls", "PBexactcalls",
      "Illexactcalls", "TenXexactcalls", "CGexactcalls",
      "HG2count", "HG3count", "HG4count",
      "NumTechs", "NumTechsExact",
      "DistBack", "DistForward", "DistMin", "DistMinlt1000",
      "MultiTech", "MultiTechExact",
      "sizecat",
      "DistPASSHG2gt49Minlt1000", "DistPASSMinlt1000",
      "MendelianError", "HG003_GT", "HG004_GT",
      "BREAKSIMLENGTH", "REFWIDENED", "REPTYPE",
      "TRall", "TRgt100", "TRgt10k", "segdup"
    )
    ...
    cnames <- c(
      cnames, "DPSum", "platforms", "platformnames", "platformbias",
      "datasets", "datasetnames", "datasetsmissingcall",
      "callsets", "callsetnames", "varType", "filt", "callable",
    ...
    cnames <- c(cnames, "TRF", "TRFdiff", "TRFrepeat", "TRFovl",
      "TRFstart", "TRFend", "TRFperiod", "TRFcopies",
      "TRFscore", "TRFentropy", "TRFsim"
    )
    ctypes <- paste0(ctypes, "cicdiiiinnn")
    if (str_detect(benchmarkset, "stvar")) {
      cnames <- c(
        cnames, "SVTYPE", "SVLEN", "RM_score", "RM_repeat",
        "RM_clsfam", "LCR", "REMAP"
      )
...
read_bench_file <- function(path, col_info) {
  ...
  var_tbl$chrom <- factor(var_tbl$chrom, 
                          levels = paste0("chr", c(1:22, "X", "Y")))

  ## Dropping extra columns
  var_tbl <- var_tbl %>%
    select(chrom, pos, end, gt, vkx, var_type, var_size, region_ids, strat_ids)
...
bench_vars_tbls <- anno_var_tbl_files %>%
  ...
  mutate(col_info = map(benchmarkset, get_bench_var_cols)) %>% 
  mutate(data = future_map2(path, col_info, read_bench_file)) %>%
...
(smvar_len_dist_plt <-  ggplot(smvar_len_df,
  aes(x = var_size)
) +
  geom_histogram(
  ...
  geom_histogram(
    data = filter(smvar_len_df, bench_version == "v4.2.1"),
    fill = "grey30", color = "grey20", width = 0.25,
    bins = 100
  ) +
  # geom_text(aes(x = 300, y = 3000, label = "Alu"), size = 5, color = "darkblue") +
  # geom_text(aes(x = -300, y = 3000, label = "Alu"), size = 5, color = "darkblue") +
  ...
  # geom_text(aes(x = -6000, y = 500, label = "LINE"), size = 5, color = "darkorange") +
  # scale_x_continuous(
  #   breaks = c(-60, -45, -30, -15, -10, -5, -1, 0, 1, 5, 20, 30, 40, 50),
  #   trans = scales::pseudo_log_trans(sigma = 1, base = 0.5),
  #   labels = scales::comma
  # ) +
  scale_y_continuous(labels = scales::comma) +
  labs(
...
%%TODO%% fix filtering and annotations
...
```{r}
(diff_var_plt <- diff_var_plt_df %>%
  filter(!is.na(strat_ids)) %>%
  mutate(var_type = if_else(bench_type == "smvar", var_type, SVTYPE)) %>%
  filter(bench_type == "smvar") %>%
  ggplot() +
  geom_bar(aes(x = var_type, fill = bench_version),
    color = "grey20", position = "dodge"
  ) +
  facet_grid(bench_type ~ strat_ids, scale = "free") +
  ...
  theme_minimal() +
  theme(legend.position = "bottom", axis.text.x = element_text(angle = -45, vjust = 1))
)
```
```{r}
...
```{r}
ggplot(diff_included_plt_df, aes(
  x = bench_type,
  y = included_pct
)) +
  geom_point(
  ...
  labs(
    x = "Benchmark Type", y = "Included Bases (%)",
...
(interval_count_plt <- regions_df %>%
  ...
  ggplot() +
  geom_bar(aes(x = bench_version, fill = bench_version),
