...
diff_cov_df <- benchmarks %>%
  ...
  # Create bench_strat column for grouping (benchmark/stratification)
  mutate(
    bench_strat = glue::glue("{benchmark}/{strat_name}")
  ) %>%
  select(-benchmark)
```
...
```{r}
# Aggregate coverage metrics by benchmark and stratification
diff_cov_total_df <- diff_cov_df %>%
  group_by(bench_strat, strat_name) %>%
  summarise(
