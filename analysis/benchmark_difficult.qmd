...
# Create variant count breakdown by genomic context from boolean columns
# Pivot boolean context columns (HP, MAP, SD, TR) to long format
genomic_context_var_df <- variants_df %>%
  tidyr::pivot_longer(
    cols = c(HP, MAP, SD, TR),
    names_to = "context_name",
    values_to = "in_context"
  ) %>%
  filter(in_context) %>%
  count(bench_version, ref, bench_type, var_type, context_name, name = "count")
```
...
(genomic_context_smvar_plt <- genomic_context_var_df %>%
  ...
  facet_wrap(. ~ context_name, nrow = 1) +
...
genomic_context_var_fld_chg <- variants_df %>%
  tidyr::pivot_longer(
    cols = c(HP, MAP, SD, TR, TR10kb, SD10kb),
    names_to = "context_name",
    values_to = "in_context"
  ) %>%
  filter(in_context) %>%
  ...
  count(bench_version, ref, bench_type, var_type, var_size_bin, context_name) %>%
  pivot_wider(
    names_from = bench_version,
    values_from = n,
    id_cols = c(ref, bench_type, var_type, var_size_bin, context_name)
  ) %>%
  filter(
...
genomic_context_var_fld_chg %>%
  filter(context_name %in% c("HP", "MAP", "SD", "TR")) %>%
  mutate(
    bench_type = factor(
      bench_type,
