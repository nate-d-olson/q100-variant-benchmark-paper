...
variants_df <- load_all_variant_tables() %>%
  filter(is_pass, region_ids == "BMKREGIONS")
...
diff_cov_df %>%
  filter(
    str_detect(bench_context, "GRCh38_smvar") |
      str_detect(bench_context, "GRCh37_stvar"),
    context_name %in% c("HP", "MAP", "SD", "TR")
  ) %>%
    separate(
    bench_context,
    into = c("bench_version", "ref", "bench_type", "context"),
  ...
  group_by(bench_version, ref, bench_type, context, fully_covered) %>%
  summarise(diff_bp = sum(interval_bp)) %>%
  ggplot() +
  geom_bar(
    aes(x = bench_version, fill = fully_covered, y = diff_bp),
    stat = "identity",
    position = position_stack(),
  ) +
  # scale_benchmark_version(aesthetic = "fill") +
  facet_grid(context ~ bench_type, scales = "free_y") +
...
(genomic_context_smvar_plt <- genomic_context_var_df %>%
  ...
  scale_y_continuous(label = scales::label_comma()) +
  scale_benchmark_version(aesthetic = "fill", drop = FALSE) +
...
(combined_context_plt <- (genomic_context_smvar_plt + theme(axis.title.x = element_blank())) +
  (genomic_context_stvar_plt ) +
  plot_layout(nrow = 2, ncol = 1, guides = "collect") & theme(legend.position = "bottom", 
      plot.margin = unit(c(5, 5, 5, 5), "pt"),
      legend.margin = margin(t = 0, r = 0, b = 0, l = 0)
    ))

ggsave(
  filename = here("manuscript", "figs", "genomic_context_variant_counts_tall.png"),
