...
full_join(bench_region_stats_df) %>%
  ...
  group_by(bench_type) %>%
  gt::gt(groupname_col = "Benchmarkset Type") %>%
  gt::fmt_integer(
    columns = c(SNV, INDEL, INS, DEL, Total, total_bp)
  ) %>%
  gt::sub_missing(
    missing_text = ""
  ) %>%
  ...
  gt::tab_spanner(
    label = "Variants",
    columns = c(SNV, INDEL, INS, DEL, Total)
  ) %>%
    gt::tab_spanner(
    label = "Regions",
    columns = c(total_bp, pct_ref_cvg, pct_q100_cvg)
  ) %>%
  theme_gt_manuscript()
```
