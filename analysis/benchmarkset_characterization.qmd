...
```{r}
# Verify expected number of metrics rows
...
assertthat::assert_that(
  ...
  msg = glue::glue("Expected {expected_rows} or more rows in strat_metrics_df, got {nrow(strat_metrics_df)}")
)

# Verify all required stratifications are present
required_strats <- c("HP", "MAP", "SD", "SD10kb", "TR", "TR10kb")
assertthat::assert_that(
  all(required_strats %in% unique(strat_metrics_df$strat_name)),
  msg = glue::glue("Missing expected stratification names")
)
...
- Columns: `bench_version`, `ref`, `bench_type`, `strat_name`, `total_variants`, `snp_count`, `indel_count`, `del_count`, `ins_count`, `complex_count`, `variant_density_per_mb`

**regions_df** - Benchmark region intervals
- All intervals from benchmark BED files
...
```{r}
# Create variant type breakdown for combined table
...
diff_var_plt_df <- strat_metrics_df %>%
  ...
  pivot_longer(
    cols = c(snp_count, indel_count, del_count, ins_count, complex_count, other_count),
    names_to = "count_type",
    values_to = "count"
  ) %>%
  mutate(
    var_type = case_when(
      count_type == "snp_count" ~ "SNP",
