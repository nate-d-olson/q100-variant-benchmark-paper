...
### Small Variant Counts Breakdown

```{r small_variant_breakdown}
# Calculate breakdown of small variants by size bin
smvar_breakdown_df <- variants_df %>%
  filter(bench_type == "smvar") %>%
  mutate(
    size_bin = if_else(abs(var_size) < 15, "<15bp", "15-49bp")
  ) %>%
  count(ref, bench_version, size_bin) %>%
  pivot_wider(names_from = size_bin, values_from = n, values_fill = 0) %>%
  mutate(Total = `<15bp` + `15-49bp`)

# Display as table
smvar_breakdown_df %>%
  arrange(ref, bench_version) %>%
  gt::gt(caption = "Breakdown of small variant counts by size (<15bp and 15-49bp).") %>%
  gt::fmt_integer(columns = c(`<15bp`, `15-49bp`, Total)) %>%
  gt::cols_align(
    columns = c("ref", "bench_version"),
    align = "right"
  ) %>%
