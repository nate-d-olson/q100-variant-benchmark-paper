...
This section quantifies the number of variants removed by each exclusion region in the v5.0q benchmark.

```{r exclusion_impact}
# Load exclusion metrics with variant counts
exclusion_df <- load_exclusion_metrics()

if (nrow(exclusion_df) > 0) {
  # Summary table of exclusion impact
  exclusion_df %>%
    filter(bench_version == "v5.0q") %>%
    select(
      ref, bench_type, exclusion, 
      total_variants, 
      # Conditionally select columns if they exist (handled by select helpers or explicit check)
      any_of(c("snv_count", "indel_count", "ins_count", "del_count"))
    ) %>%
    arrange(ref, bench_type, desc(total_variants)) %>%
    group_by(ref, bench_type) %>%
    gt::gt() %>%
    gt::fmt_integer(columns = where(is.integer)) %>%
    gt::cols_label(
      exclusion = "Exclusion Region",
    ...
    gt::tab_header(
      title = "Variants Removed by Exclusion Regions (v5.0q)"
    )
} else {
  print("No exclusion data available.")
}
```
...
(coverage_change_plt <- benchmark_regions_df %>%
    filter(
        bench_version %in% c("v5.0q", "v4.2.1", "v0.6"),
