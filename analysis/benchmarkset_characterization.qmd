...
bench_var_cnt_df %>% 
  rowwise() %>% 
  mutate(Total = sum(SNV, INDEL, INS, DEL, na.rm = TRUE)) %>% 
full_join(bench_region_stats_df) %>%
  select(-bench_version) %>%
  ungroup() %>%
  mutate(
    ref = std_references(ref),
    bench_type = get_bench_type_labels()[as.character(bench_type)],
    across(c(SNV, INDEL, INS, DEL), ~ replace_na(., 0L))
  ) %>%
  arrange(bench_type, ref) %>%
  select(bench_type, ref, SNV, INDEL, INS, DEL, total_bp, pct_ref_cvg, pct_q100_cvg)  %>%
  as_grouped_flextable(groupname_col = "bench_type") %>%
  fmt_integer_flextable(columns = c("SNV", "INDEL", "INS", "DEL", "total_bp")) %>%
