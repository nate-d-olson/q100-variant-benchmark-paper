...
assertthat::assert_that(
  ...
  msg = glue::glue(
    ...
    "got {nrow(strat_metrics_df)} (from config: {expected_benchmarks} benchmarks × {expected_contexts} contexts)"
  )
)
...
assertthat::assert_that(
  all(required_contexts %in% levels(strat_metrics_df$context_name)),
...
```{r}
# Aggregate small variant counts by type across all genomic contexts
# Note: snv_count is renamed from snp_count in load_genomic_context_metrics()
...
diff_var_plt_df <- strat_metrics_df %>%
  ...
  mutate(
    ...
    context_id = context_name  # Use genomic context name as grouping variable
  ) %>%
