...
(smvar_cnt_df <- bench_vars_tbls %>%
    ...
    count(name = "nvars") %>% 
   mutate(var_type = if_else(var_type == "SNP", "SNV", var_type)))
```
...
(stvar_cnt_table <- stvar_cnt_df %>%
    ...
    arrange(ref) %>%
    ungroup() %>%
    gt::gt(caption = "Number of insertion, deletion, and total structural variants ( >49bp) by benchmark set.") %>%
    gt::fmt_integer(
...
```{r}
bind_rows(smvar_cnt_df, stvar_cnt_df) %>% colnames()
```

```{r}
combined_var_cnt_df <- bind_rows(smvar_cnt_df, stvar_cnt_df) %>%
   filter(bench_version == "v5.0q") %>% 
   select(-bench_version) %>% 
  pivot_wider(names_from = ref, values_from = nvars, 
              id_cols = c(bench_type, var_type)) %>% 
   ungroup() %>% 
   group_by(bench_type) %>%
...
var_cnt_gt <- gt(combined_var_cnt_df, 
  ...
  tab_stubhead(label = "Benchmark Type") %>%
  tab_row_group(
...
(diff_stvar_plt <- diff_var_plt_df %>%
    ...
    facet_wrap(vars(bench_type, strat_ids), nrow = 2, scales = "free") +
    scale_y_continuous(label = scales::label_comma()) +
    # scale_fill_manual(
    #     values = c(
    #         "v5.0q" = "darkgreen",
...
```
```{r}
ggplot(diff_var_fld_chg %>% filter(strat_ids %in% c("HP", "MAP", "SD", "TR"))) +
  ...
  scale_fill_viridis_(
     = c("<15bp", "15-49bp", "50bp-1kb", "1kb-10kb", ">10kb"),
    guide = guide_legend(override.aes = list(shape = 21))
  ) +
  scale_y_continuous(labels = scales::comma) +
  labs(
    x = "Difficult Genomic Context",
    y = "Fold Change Number of Variants",
    fill = "Variants Type",
    shape = "Benchmark Set"
  ) +
  theme_minimal() +
...
```{r eval = FALSE}
  ...
  # {set_names(., str_extract(., "(?<=diff_region_coverage/).*(?=_cov.bed)"))} %>% 
)
  # map_dfr(
  #   read_tsv, .id = cov_bed
  # ) %>%
