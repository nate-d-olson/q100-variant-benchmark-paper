...
- Generated by `generate_variant_parquet.py` using Truvari's `vcf_to_df()`
...
The pipeline uses Truvari's `vcf_to_df()` to read VCFs and classify variant types in `workflow/scripts/generate_variant_parquet.py`:

**For small variants (smvar, <50bp):**

- Truvari svtype enum 0 (SNP) → SNV
- All other types → INDEL

**For structural variants (stvar, >=50bp):**

- Truvari svtype enum 1 (DEL) → DEL
- Truvari svtype enum 2 (INS) → INS
- DUP (3), INV (4), UNK (6) → reclassified as INS or DEL based on allele size (ALT > REF → INS, else DEL)
- NON (5) is filtered out before classification

**Size filtering:** smvar keeps abs(svlen) < 50; stvar keeps abs(svlen) >= 50

**Pipeline flow:**

1. `annotate_vcf_genomic_contexts` — bcftools annotate adds CONTEXT_IDS/REGION_IDS to VCF
2. `generate_variant_parquet` — Truvari reads annotated VCF, classifies types, writes Parquet
