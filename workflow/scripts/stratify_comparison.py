...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(shared_bed_tmp, "w") as f:
            subprocess.run(
                ["bedtools", "intersect", "-a", new_bed, "-b", old_bed], stdout=f
            )

        with open(unique_new_bed_tmp, "w") as f:
            subprocess.run(
                ["bedtools", "subtract", "-a", new_bed, "-b", old_bed], stdout=f
            )

        with open(unique_old_bed_tmp, "w") as f:
            subprocess.run(
                ["bedtools", "subtract", "-a", old_bed, "-b", new_bed], stdout=f
            )
        ...
        for strat_bed in strat_beds:
            strat_name = (
                os.path.basename(strat_bed).replace(".bed.gz", "").replace(".bed", "")
            )
            ...
            s_unique_new = run_bedtools_intersect_bed_bases(
                unique_new_bed_tmp, strat_bed
            )
            s_unique_old = run_bedtools_intersect_bed_bases(
                unique_old_bed_tmp, strat_bed
            )
