    ...
    with open(log_path, 'w') as log:
        ...
        try:
            with open(variant_table_path, 'r') as f:
                ...
                missing = []
                if not chrom_col: missing.append("CHROM")
                if not type_col: missing.append("TYPE")
                if not strat_col: missing.append("STRAT_IDS")
                
                if missing:
                    raise ValueError(f"Missing required columns: {missing}")

                log.write(f"Using columns: TYPE='{type_col}', STRAT_IDS='{strat_col}'\n\n")
