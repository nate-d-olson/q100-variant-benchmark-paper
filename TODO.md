...
- [ ] exclusion figure move to methods or supplemental. Bigger text: split into two plots and increased font size. Will want to revise some more after finalizing which figures to include.
- [ ] Ideogram
...
- [ ] Chr 8 inversion, larger text, cut top to 100Mb, fix truncated legend.: refined a bit will want to fix annotations (include MAT,PAT, and GRCh38) and decide whether to include full chromosome after finalizing which figures to include.
...
- [ ] rerun test surfaced error with external_evaluation.qmd likely issue with not having the correct evaluation input file but could also be bug in R code. Will want to check later, not pressing though.
- [ ] Curate unsure SVs
- [ ] Tool for snakemake pipeline diagram https://github.com/OpenOmics/snakevision
...
- [x] Fix R tests and wire into CI
  - Updated the `var_size` assertion to the current `int64` schema.
  - Updated pre-refactor paths and variant-count normalization.
  - Added an `r-tests` job using the locked renv environment.
