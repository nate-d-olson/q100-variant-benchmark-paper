...
**Note:** Count columns for a variant type absent from a given `bench_type` (e.g., `indel_count` for `stvar` benchmarks) are `0`, not `NA` — these represent an absent variant class, not a missing measurement.
...
| `ref` | Factor | Reference genome |
| `bench_type` | Factor | Benchmark set type |
| `exclusion_combination` | Character | Pipe-delimited exclusion names (e.g., `segdups` and `gaps`) |
...
| `gt`            | Character | Genotype string from `truvari.get_gt()` e.g. `HET`, `HOM`, `REF`, `NON`, `UNK` |
| `var_type`      | Character | Variant type from `VariantRecord.var_type()` - `SNP`, `DEL`, `INS`, `DUP`, `INV`, `BND`, or `UNK` |
