...
**Usage Notes:**
- SLOW to load on first access, but automatically cached as Parquet for fast reloads
...
- Set `use_cache = FALSE` to skip caching, or `force_refresh = TRUE` to reload from source
...
Cache files are stored in `analysis/cache/` with zstd compression. The cache automatically invalidates when source files change (based on file modification times).

```r
# First load: reads from pipeline output, writes cache
variants <- load_variant_table("v5.0q_GRCh38_smvar")

# Second load: reads from Parquet cache (fast)
variants <- load_variant_table("v5.0q_GRCh38_smvar")
...
cache_info()                        # List cached files with size and age
invalidate_cache("variant_table")   # Remove cache for one dataset
...
- Consider using `load_genomic_context_metrics()` instead for summary statistics
- Use `cache_info()` to verify cached files exist; use `force_refresh = TRUE` to re-cache
