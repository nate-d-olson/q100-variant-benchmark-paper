...
2. **Characterize variant distributions** by type (SNPs, indels, SVs)
3. **Calculate overlap** with genomic stratification contexts (tandem repeats, homopolymers, segmental duplications, low mappability)
4. **Generate exclusion metrics** showing regions excluded from the benchmark and their impact
5. **Compare with historical benchmarks** (v4.2.1, CMRG, v0.6 SV, TR benchmarks)

## Key Outputs

- Per-chromosome variant statistics (`results/vcfstats/`)
- SV length distributions (`results/sv_len/`)
- Context-stratified variant counts (`results/context_counts/`)
- Benchmark region coverage by genomic context (`results/context_coverage/`)
